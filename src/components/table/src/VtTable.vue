<template>
  <div
    ...
    <div v-if="isLoading" class="vt-loading vt-center-align">
      <slot name="loadingContent">
        <span class="vt-loading__content">
          Loading...
        </span>
      </slot>
    </div>
    <div class="vt-inner-wrap"
      ...
      >
        <vt-pagination
          ...
          :perPageDropdownEnabled="pageOptions.perPageDropdownEnabled"
      ...
      <div
        ...
        class="vt-selection-info-row clearfix"
      ...
      >
        ...
        <div class="vt-selection-info-row__actions vt-pull-right">
          <slot name="selected-row-actions">
          </slot>
        </div>
      </div>
      <div class="vt-fixed-header">
        <table
          id="vt-table"
          v-if="tableOptions.fixedHeader"
        ...
        <colgroup>
          ...
          <vt-table-header
            ...
            :columns="columns"
            :line-numbers="tableOptions.lineNumbers"
      ...
      <div
        :class="{'vt-responsive': responsive}"
        :style="wrapperStyles"
      >
        <table
          id="vt-table"
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  <td 
                    v-if="$slots[`body-actions-${count}`]"
                    :class="getActionsClasses(i, 'td')"
                  >
                    <slot :name="`body-actions-${count}`" :row="row"></slot>
                  </td>
                </template>
              </tr>
            </template>
          ...
          <tbody v-if="showEmptySlot">
            <tr>
              <td :colspan="fullColspan">
                <div class="vt-center-align vt-text-disabled">
                  <slot name="no-data">
                    No Data
                  </slot>
                </div>
      ...
      <div v-if="hasFooterSlot" class="vt-wrap__actions-footer">
...
import VtPagination from './pagination/VtPagination.vue';
import VtGlobalSearch from './VtGlobalSearch.vue';
import VtTableHeader from './VtTableHeader.vue';
import VtHeaderRow from './VtHeaderRow.vue';
...
export default {
  ...
  computed: {
    ...
    fullColspan() {
      ...
      if (this.tableOptions.lineNumbers) fullColspan++;
  ...
  methods: {
    ...
    getActionsClasses(index, element) {
      let oldLen = this.typedColumns.length
      let valueColumns = this.typedColumns.filter(t => t.actions != true)
      let newLen = valueColumns?.length
      let subLen = oldLen - newLen
      let currentColumn = this.typedColumns[newLen + index]
      let isAction = currentColumn?.actions || false
      if (!isAction) return
      let fixed = currentColumn?.fixed
      const classes = {
        'vt-fixed-right': fixed === 'right',
        'vt-fixed-left': fixed === 'left', 
      }
    ...
    initializeSort() {
      ...
      if (typeof initSortBy === 'object') {
        const ref = this.tableOptions.fixedHeader
  ...
  components: {
    ...
    'vt-global-search': VtGlobalSearch,
    'vt-header-row': VtHeaderRow,
    'vt-table-header': VtTableHeader,
  },
};
</script>

...
const ICONS = {
  ...
  close:
    '<svg width="16" height="16" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg"><rect width="48" height="48" fill="white" fill-opacity="0.01"/><path d="M14 14L34 34" stroke="#909399" stroke-width="8" stroke-linecap="round" stroke-linejoin="round"/><path d="M14 34L34 14" stroke="#909399" stroke-width="8" stroke-linecap="round" stroke-linejoin="round"/></svg>',
}
...
function Msg(opts) {
  ...
  oMsg.id = VtMessage.instanceCount
  ...
  let $svg = ICONS[oMsg.settings.type || 'info']
  let $type = oMsg.settings.type
  let contentClassName = namespacify('content-' + $type || 'info')
  contentClassName += oMsg.settings.showClose
    ? ' ' + namespacify('content-with-close')
    : ''
  let content = oMsg.settings.content || ''
  let details = oMsg.settings.details || ''
  let $closeSvg = ICONS['close']
  let $closeIcon = oMsg.settings.showClose
    ? '<i class="vt-message-icon vt-message-icon-close">' + $closeSvg + '</i>'
    : ''
  ...
  $elem.innerHTML = `
    <div class="vt-message-content ${$type}">
      <div class="${contentClassName}">
        <i class="vt-message-icon">
          ${$svg}
        </i>
        ${$span.outerHTML}
        ${$closeIcon}
      </div>
      <div class="vt-message-details">
        ${$more.outerHTML}
      </div>
    </div>
  `
  ...
  if (oMsg.settings.showClose) {
    $elem.querySelector('.vt-message-icon-close').addEventListener(
      'click',
      function () {
        oMsg.close()
      }.bind($elem)
...
function setMsgCount(oMsg) {
  ...
  var $content = oMsg.$elem.querySelector('[class^="vt-message-content-"]'),
...
const VtMessage = {
  ...
  config: function (cfg) {
    ...
    this.maxNums =
      DEFAULTS.maxNums && DEFAULTS.maxNums > 0 ? parseInt(DEFAULTS.maxNums) : 3
  },

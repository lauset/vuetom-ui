<template>
  <div ref="viewer" :class="jvClass">
    <div v-if="copyable" :class="`vt-jv-tooltip ${copyText.align || 'right'}`">
      <span ref="clip" class="vt-jv-button" :class="{ copied }">
        <slot name="copy" :copied="copied">
          {{ copied ? copyText.copiedText : copyText.copyText }}
        </slot>
      </span>
    </div>
    <div class="vt-jv-code" :class="{ open: expandCode, boxed }">
      <json-box
        ref="jsonBox"
        :value="value"
        :sort="sort"
    ...
    >
      <span class="vt-jv-toggle" :class="{ open: !!expandCode }" />
    </div>
  </div>
</template>
...
import JsonIcon from './json-icon.vue'
...
export default {
  ...
  computed: {
    jvClass() {
      return 'vt-jv-container ' + this.theme + (this.boxed ? ' boxed' : '')
    },
    copyText() {
...
.vt-jv-container {
  ...
  .vt-jv-toggle {
    background-image: url(data:image/svg+xml;base64,PHN2ZyBoZWlnaHQ9IjE2IiB3aWR0aD0iOCIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj4KPHBvbHlnb24gcG9pbnRzPSIwLDAgOCw4IDAsMTYiCnN0eWxlPSJmaWxsOiM2NjY7c3Ryb2tlOnB1cnBsZTtzdHJva2Utd2lkdGg6MCIgLz4KPC9zdmc+);
